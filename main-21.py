...
def get_ticker(code):
    ...
    # If explicitly in US list, return as-is
    if code in US_STOCKS: return code
    # If looks like IDX (letters only, known IDX pattern)
    if code in IDX_STOCKS: return f"{code}.JK"
    # Try to detect: if has digits or >4 chars common in US, try US first
    # Default: try .JK first, fallback logic in get_signal
    return f"{code}.JK"
...
def get_signal(code,tf="D"):
    ...
    try:
        ...
        # Auto-fallback: if IDX (.JK) has no data, try as US stock
...
def generate_chart(code, tf="D", volume_spikes=None):
    ...
    # Auto detect swing high & low from last n candles
    swing_high = max(highs)
    swing_low  = min(lows)
    fib_range  = swing_high - swing_low

    fib_levels = {
        "0.0":   (swing_high,        "#ffffff", "0% High"),
        ...
        "100.0": (swing_low,          "#ff4444", "100% Low"),
    }
    ...
    fib_styles = {
        ...
        "38.2":  (0.8, "-."),
        "50.0":  (0.8, "-."),
        "61.8":  (1.2, "-"),   # Golden ratio - paling tebal
        "78.6":  (0.8, "-."),
        "100.0": (0.5, "--"),
    }
    ...
    for key,(fval,fcol,flabel) in fib_levels.items():
        ...
        ax1.text(0.5, fval, f" {flabel} {fval:,.2f}",
                color=fcol, fontsize=6.5, va='center', alpha=0.85,
                bbox=dict(boxstyle='round,pad=0.1', facecolor=BG, edgecolor='none', alpha=0.6))
