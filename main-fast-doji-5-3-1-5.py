...
async def help_cmd(u,c):
    await u.message.reply_text(
        ...
        "📐 *Pattern Screener (BARU):*\n"
        "`/pattern` — Scan IDX: Falling Wedge, Cup&Handle, H&S, Double Bottom\n"
        "`/pattern us` — Scan US stocks\n"
        "`/pattern idx 4h` — Bisa pilih TF juga\n"
        "🤖 Auto alert pattern breakout tiap 1 jam saat market buka\n\n"
        "*Auto Scan:*\n"
...
# ══════════════════════════════════════════
# PATTERN SCREENER — Falling Wedge, Cup&Handle, H&S, Double Bottom
# ══════════════════════════════════════════
def scan_pattern_one(code, tf="D"):
    """Scan 1 saham untuk semua pola chart -> return list hasil (bisa lebih dari 1 pola)"""
    r = get_signal(code, tf)
    if "error" in r:
        return []
    hits = detect_all_patterns(r["df"])
    out = []
    for h in hits:
        h["code"] = code
        h["ticker"] = r["ticker"]
        h["price"] = r["price"]
        h["chg"] = r["chg"]
        h["liquid"] = r.get("liquid", True)
        h["tf"] = tf
        out.append(h)
    return out

def pattern_screener(stock_list, tf="D", max_workers=10):
    """Scan pattern seluruh daftar saham secara paralel"""
    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(scan_pattern_one, code, tf): code for code in stock_list}
        for future in as_completed(futures):
            try:
                res = future.result(timeout=20)
                if res:
                ...
                log.warning(f"Pattern scan error: {e}")
    # Prioritas: yang udah BREAKOUT dulu, baru urut score tertinggi
    results.sort(key=lambda x: (x["confirmed"], x["score"]), reverse=True)
    return results

def fmt_pattern_msg(results, market_name="IDX"):
    """Format pesan Telegram hasil pattern screener"""
    now_str = datetime.now(WIB).strftime("%d-%b-%Y %H:%M WIB")
    lines = [f"📐 *PATTERN SCREENER — {market_name}*", f"🕐 {now_str}",
             "━━━━━━━━━━━━━━━━━━━━"]
    if not results:
        lines.append("✅ Tidak ada pola falling wedge / cup&handle / H&S / double bottom terdeteksi saat ini.")
    else:
        confirmed = [r for r in results if r["confirmed"]]
        watching  = [r for r in results if not r["confirmed"]]
        if confirmed:
            lines.append(f"\n🔥 *BREAKOUT TERKONFIRMASI:* ({len(confirmed)})")
            for r in confirmed[:8]:
                is_idr = r["ticker"].endswith(".JK")
                px = f"Rp {r['price']:,.0f}" if is_idr else f"${r['price']:,.2f}"
                liq = " ⚠️" if not r.get("liquid", True) else ""
                lines.append(
                    f"  {r['emoji']} *{r['code']}* `{px}` {r['chg']:+.2f}%\n"
                    f"    ↳ {r['pattern']} | {r['status']} | Vol:`{r['vr']:.1f}x`{liq}"
                )
        if watching:
            lines.append(f"\n⏳ *MENDEKATI BREAKOUT (watchlist):* ({len(watching)})")
            for r in watching[:8]:
                is_idr = r["ticker"].endswith(".JK")
                px = f"Rp {r['price']:,.0f}" if is_idr else f"${r['price']:,.2f}"
                liq = " ⚠️" if not r.get("liquid", True) else ""
                lines.append(
                    f"  {r['emoji']} *{r['code']}* `{px}` {r['chg']:+.2f}%\n"
                    f"    ↳ {r['pattern']} | {r['status']}{liq}"
                )
    lines += ["━━━━━━━━━━━━━━━━━━━━",
              "💡 Falling Wedge & Cup/Handle & Double Bottom & Inverse H&S = bullish",
              "⚠️ H&S normal (kepala di atas) = bearish, hati-hati",
              "⚠️ Pola chart HANYA sinyal awal, selalu cek volume & konfirmasi candle!",
              f"⏱ {now_str}"]
    return "\n".join(lines)

async def pattern_cmd(u, c):
    """Command /pattern [us|idx] [tf] — scan pola chart breakout"""
    args = c.args
    market = "idx"
    tf = "D"
    for a in args:
        ...
        elif a.upper() in TF_MAP: tf = a.upper()
    stock_list = US_STOCKS if market == "us" else IDX_STOCKS
    market_name = "US" if market == "us" else "IDX"
    flag = "🇺🇸" if market == "us" else "🇮🇩"

    m = await u.message.reply_text(
        f"📐 Scanning *Pattern* {flag} {market_name} TF:{tf}...\n"
        f"Falling Wedge, Cup&Handle, Head&Shoulders, Double Bottom\n"
        f"⏳ Mohon tunggu (parallel scan ⚡)", parse_mode="Markdown")
    try:
        results = await asyncio.get_event_loop().run_in_executor(
            None, pattern_screener, stock_list, tf)
        msg = fmt_pattern_msg(results, market_name)
        await m.edit_text(msg, parse_mode="Markdown")

        # Kirim chart untuk top pick (prioritas yang sudah breakout & liquid)
        confirmed_liquid = [r for r in results if r["confirmed"] and r.get("liquid", True)]
        best = confirmed_liquid[0] if confirmed_liquid else (results[0] if results else None)
        if best:
            buf, _ = generate_chart(best["code"], tf)
            if buf:
                is_idr = best["ticker"].endswith(".JK")
                px = f"Rp {best['price']:,.0f}" if is_idr else f"${best['price']:,.2f}"
                await u.message.reply_photo(
                    photo=buf,
                    caption=(f"{best['emoji']} *{best['code']}* | TF:{tf} | `{px}` {best['chg']:+.2f}%\n"
                             f"📐 {best['pattern']} | {best['status']}\n"
                             f"Vol:`{best['vr']:.1f}x` | Bias:*{best['bias']}*"),
                    parse_mode="Markdown")
    except Exception as e:
        await m.edit_text(f"❌ Error pattern scan: {e}")

async def pattern_auto_scan(context: ContextTypes.DEFAULT_TYPE):
    """Auto-scan berkala, kirim ke user yang /auto aktif kalau ada breakout terkonfirmasi"""
    if not is_weekday(): return
    if not (is_idx_market_open() or is_us_market_open()): return
    bot = context.bot
    try:
        idx_results = await asyncio.get_event_loop().run_in_executor(
            None, pattern_screener, IDX_STOCKS, "D")
        confirmed = [r for r in idx_results if r["confirmed"] and r.get("liquid", True)]
        if not confirmed: return
        for uid in list(auto_users.keys()):
            try:
                msg = fmt_pattern_msg(confirmed[:5], "IDX (Auto)")
                await bot.send_message(int(uid), msg, parse_mode="Markdown")
            except Exception as e:
                log.warning(f"pattern auto send error uid {uid}: {e}")
    except Exception as e:
        log.error(f"pattern_auto_scan error: {e}")
...
def run_bot():
    ...
    # pattern screener (falling wedge/cup&handle/H&S/double bottom): tiap 1 jam
    jq.run_repeating(pattern_auto_scan,interval=3600,first=1200)
    now=datetime.now(WIB)
    if now.weekday()>=5:
