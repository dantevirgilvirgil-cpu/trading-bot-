...
def generate_chart(code, tf="D", volume_spikes=None):
    ...
    # ══ is_idr & price_fmt — definisi awal (dipakai Donchian & Fibonacci) ══
    is_idr=r["ticker"].endswith(".JK")
    price_fmt=lambda p: f"Rp {p:,.0f}" if is_idr else f"${p:,.2f}"
    ...
    try:
        ...
        # Price labels di ujung kanan
        last_x = n - 1
        ax1.text(last_x + 0.4, dc_upper_v[-1],
                 f" {price_fmt(dc_upper_v[-1])}",
                 color=DC_UPPER_COLOR, fontsize=6, va='center', fontweight='bold',
                 bbox=dict(boxstyle='round,pad=0.15', facecolor='white',
                           edgecolor=DC_UPPER_COLOR, alpha=0.75, linewidth=0.6),
                 zorder=11)
        ax1.text(last_x + 0.4, dc_lower_v[-1],
                 f" {price_fmt(dc_lower_v[-1])}",
                 color=DC_LOWER_COLOR, fontsize=6, va='center', fontweight='bold',
                 bbox=dict(boxstyle='round,pad=0.15', facecolor='white',
                           edgecolor=DC_LOWER_COLOR, alpha=0.75, linewidth=0.6),
                 zorder=11)
