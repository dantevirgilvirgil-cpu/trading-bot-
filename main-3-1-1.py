...
TF_MAP={"5M":("5m","5d"),"15M":("15m","5d"),"30M":("30m","10d"),
        "1H":("60m","60d"),"4H":("60m","60d"),"D":("1d","1y"),"1D":("1d","1y"),
        "W":("1wk","5y"),"1W":("1wk","5y"),
        "M":("1mo","10y"),"1M":("1mo","10y")}
...
def hull_ma(s,p=20):
    """Hull MA — kandidat 'garis magenta' ala T1MO: nempel harga tapi tetap smooth."""
    half=int(p/2); sq=max(2,int(np.sqrt(p)))
    w1=s.rolling(half).apply(lambda x: np.dot(x,np.arange(1,half+1))/np.arange(1,half+1).sum(), raw=True)
    w2=s.rolling(p).apply(lambda x: np.dot(x,np.arange(1,p+1))/np.arange(1,p+1).sum(), raw=True)
    raw=2*w1-w2
    return raw.rolling(sq).apply(lambda x: np.dot(x,np.arange(1,sq+1))/np.arange(1,sq+1).sum(), raw=True)

def composite_strength(le9,le20,le50,lc,lr,lm,ls,vr,lsk,dc_pos=None):
    """
    Composite Strength Score (0-100), pendekatan konsep 'Strength to COMPOSITE' T1MO.
    Blend: trend stack(40%) + momentum(30%) + volume(15%) + posisi channel/'lautan'(15%).
    dc_pos = posisi close dalam Donchian channel (0-100), opsional.
    """
    trend = sum([lc>le9, le9>le20, le20>le50, lc>le50]) / 4 * 100
    macd_norm = (np.tanh((lm-ls)/max(abs(lc)*0.01,1e-6))*50+50)
    momentum = min(max(lr,0),100)*0.6 + macd_norm*0.4
    volume = min(vr,3)/3*100
    if dc_pos is None:
        w = [0.40,0.35,0.25]; comp = trend*w[0]+momentum*w[1]+volume*w[2]
    else:
        comp = trend*0.40+momentum*0.30+volume*0.15+dc_pos*0.15
    comp = round(min(max(comp,0),100),1)
    if comp>=75: lbl="🟢 STRONG"
    elif comp>=60: lbl="🟢 BULLISH"
    elif comp>=40: lbl="🟡 NETRAL"
    elif comp>=25: lbl="🔴 LEMAH"
    else: lbl="🔴 WEAK"
    return comp,lbl

def find_support_resistance(highs, lows, closes, lookback=3, n_levels=4):
    """
    Deteksi level Support/Resistance dari pivot high/low (fractal),
    ala S1-S4/R1-R4 T1MO. Dihitung dari window candle yang sama dgn chart.
    """
    n=len(closes)
    piv_high=[]; piv_low=[]
    for i in range(lookback, n-lookback):
        wh=highs[i-lookback:i+lookback+1]; wl=lows[i-lookback:i+lookback+1]
        if highs[i]==max(wh): piv_high.append(round(float(highs[i]),2))
        if lows[i]==min(wl):  piv_low.append(round(float(lows[i]),2))
    cur=closes[-1]
    res=sorted(set(x for x in piv_high if x>cur))[:n_levels]
    sup=sorted(set(x for x in piv_low if x<cur), reverse=True)[:n_levels]
    return sup,res

def get_ticker(code):
    code=code.upper().replace(".JK","").replace("-","")
    if code in ("IHSG","JKSE","COMPOSITE"): return "^JKSE"
...
def get_signal(code,tf="D"):
    ...
    try:
        ...
        if (df.empty or len(df)<26) and ticker.endswith(".JK") and ticker!="^JKSE":
            ticker=code.upper()
            df = get_cached_data(ticker, iv, per)
        ...
        is_idx = (ticker.endswith(".JK") or ticker=="^JKSE")
        liquid = is_liquid_stock(av, lc) if is_idx else True
        liquidity_tag = "" if liquid else "⚠️ LOW LIQUIDITY"
        # ── Composite Strength (pendekatan 'Strength to COMPOSITE' T1MO) ──
        try:
            dc_hi=float(h.rolling(20).max().iloc[-1]); dc_lo=float(l.rolling(20).min().iloc[-1])
            dc_pos=min(max((lc-dc_lo)/(dc_hi-dc_lo)*100,0),100) if dc_hi>dc_lo else 50
        except Exception:
            dc_pos=None
        comp_score,comp_label=composite_strength(le9,le20,le50,lc,lr,lm,ls,vr,lsk,dc_pos)
        return{"code":code.upper(),"ticker":ticker,"tf":tf.upper(),"price":lc,"chg":chg,
               ...
               "composite":comp_score,"composite_label":comp_label,
...
def fmt_doji_msg(results_by_tf, market_name="IDX"):
    ...
    for tf in ["1H", "4H", "D"]:
        ...
        for h in hits[:6]:
            is_idr = (h["ticker"].endswith(".JK") or h["ticker"]=="^JKSE")
...
def detect_falling_wedge(highs, lows, closes, n=40):
    """
    Deteksi Falling Wedge (bullish reversal/continuation).
    Ciri: higher-high & lower-low sama2 turun, TAPI garis atas (resistance)
    turun LEBIH LANDAI dari garis bawah (support) -> makin lama makin menyempit.
    Beda dari Descending Triangle: di situ garis bawah FLAT, di wedge garis
    bawah juga ikut turun (cuma lebih curam).
    Return: dict {type, quality, breakout_level, confirmed, ...} atau None
    """
    try:
        from scipy import stats as sp_stats
        h = highs[-n:]; l = lows[-n:]; c = closes[-n:]
        ...
        # Kedua garis harus sama-sama turun (falling)
        if not (us < 0 and ls < 0): return None
        # Garis atas harus lebih landai dari garis bawah -> konvergen (menyempit)
        if not (us > ls * 1.05): return None

        last_i = len(c) - 1
        upper_now = us * last_i + ui
        lower_now = ls * last_i + li
        if upper_now <= lower_now: return None  # garis udah ketemu / invalid

        width_ratio = (upper_now - lower_now) / upper_now if upper_now else 1
        if width_ratio > 0.35: return None  # belum cukup menyempit

        # Pola harus punya penurunan yang berarti (bukan sideways biasa)
        total_decline = (uy[0] - upper_now) / uy[0] if uy[0] else 0
        if total_decline < 0.08: return None  # minimal turun 8% dari awal wedge

        current = float(c[-1])
        confirmed = current > upper_now * 1.005

        return {
            "type": "Falling Wedge 📉➡️📈",
            "quality": "BULLISH BREAKOUT" if confirmed else "BREAKOUT PENDING",
            "upper_slope": us, "upper_intercept": ui,
            "lower_slope": ls, "lower_intercept": li,
...
def detect_bullish_flag(highs, lows, closes, n=40):
    """
    Deteksi Bullish Flag (continuation pattern).
    Ciri: ada "tiang" (pole) berupa kenaikan tajam, lalu diikuti konsolidasi
    ketat berbentuk channel sejajar yang cenderung flat/turun sedikit
    (melawan arah tiang) — beda dari Falling Wedge karena garis atas &
    bawah di flag relatif SEJAJAR (channel), bukan konvergen ke satu titik.
    Konfirmasi: breakout di atas garis atas flag.
    Target proyeksi: breakout_level + tinggi tiang (measured move).
    Return: dict {type, quality, breakout_level, target, confirmed, ...} atau None
    """
    try:
        from scipy import stats as sp_stats
        h = highs[-n:]; l = lows[-n:]; c = closes[-n:]

        flag_n = max(8, int(n * 0.35))   # bagian belakang = area konsolidasi (flag)
        pole_n = n - flag_n              # bagian depan = area tiang (pole)
        if pole_n < 5 or flag_n < 6: return None

        pole_c = c[:pole_n]
        flag_h = h[-flag_n:]; flag_l = l[-flag_n:]

        # ── Pole: cari kenaikan tajam dari swing low ke harga di awal flag ──
        pole_low  = float(np.min(pole_c))
        pole_high = float(c[pole_n - 1])
        if pole_low <= 0: return None
        pole_gain_pct = (pole_high - pole_low) / pole_low * 100
        if pole_gain_pct < 8: return None   # minimal tiang naik 8%

        # ── Flag: fit garis atas & bawah dari channel konsolidasi ──
        fx = np.arange(flag_n)
        us, ui, _, _, _ = sp_stats.linregress(fx, flag_h)
        ls, li, _, _, _ = sp_stats.linregress(fx, flag_l)

        upper_now = us * (flag_n - 1) + ui
        lower_now = ls * (flag_n - 1) + li
        if upper_now <= lower_now: return None

        # Slope flag harus flat s/d agak turun (istirahat sejenak, bukan lanjut naik tajam / bukan longsor)
        slope_pct = (us / pole_high * 100) if pole_high else 0
        if slope_pct > 1.0: return None     # masih naik kencang -> masih bagian tiang, bukan flag
        if slope_pct < -6.0: return None    # turun terlalu tajam -> breakdown, bukan flag

        # Channel harus ketat (tidak melebar seperti triangle/wedge)
        channel_width_pct = (upper_now - lower_now) / upper_now * 100
        if channel_width_pct > 15: return None
        if channel_width_pct < 0.3: return None

        current = float(c[-1])
        confirmed = current > upper_now * 1.005
        pole_height = pole_high - pole_low
        target = upper_now + pole_height
        ...
        return {
            ...
            "target": float(target),
            "confirmed": confirmed,
            "n_used": n, "flag_n": flag_n, "pole_n": pole_n,
        }
    except:
        return None
...
def check_breakout_volume(vols, lookback=20, threshold=1.5):
    """
    Cek apakah volume di candle terakhir (breakout candle) cukup besar
    dibanding rata-rata volume `lookback` hari sebelumnya.
    Breakout pola yang beneran digerakkan institusi/algo besar biasanya
    dibarengi volume gede — breakout tanpa volume gede lebih rawan
    fakeout/bull trap.
    Return: (vol_ratio, vol_confirmed)
    """
    try:
        v = np.asarray(vols, dtype=float)
        if len(v) < 2: return 0.0, False
        lb = min(lookback, len(v) - 1)
        avg_vol = float(np.mean(v[-(lb + 1):-1]))
        last_vol = float(v[-1])
        if avg_vol <= 0: return 0.0, False
        ratio = last_vol / avg_vol
        return ratio, (ratio >= threshold)
    except:
        return 0.0, False

def detect_all_patterns(df, n=60, vol_threshold=1.5):
    """
    Jalankan semua pattern detector sekaligus + konfirmasi volume breakout.
    Return dict: {trendlines, triangle, falling_wedge, bullish_flag,
                  double_bottom, cup_handle, hs}
    Tiap pattern yang punya breakout level akan dapat tambahan field
    vol_ratio & vol_confirmed (volume breakout vs rata2 20 hari).
    """
    close = df["Close"].squeeze().values[-n:]
    high  = df["High"].squeeze().values[-n:]
    low   = df["Low"].squeeze().values[-n:]
    vol   = df["Volume"].squeeze().values[-n:]
    ...
    try: result["falling_wedge"] = detect_falling_wedge(high, low, close, min(n, 40))
    except: result["falling_wedge"] = None
    try: result["bullish_flag"]  = detect_bullish_flag(high, low, close, min(n, 40))
    except: result["bullish_flag"] = None
    ...
    # ── Volume confirmation: tempel ke semua pattern yang punya "confirmed" ──
    vol_ratio, vol_ok = check_breakout_volume(vol, lookback=20, threshold=vol_threshold)
    for key in ("falling_wedge", "bullish_flag", "double_bottom", "cup_handle", "hs"):
        pat = result.get(key)
        if isinstance(pat, dict) and "confirmed" in pat:
            pat["vol_ratio"] = vol_ratio
            pat["vol_confirmed"] = vol_ok
            # Breakout price-nya udah confirmed, TAPI kalau volume ga ikut gede -> anggap belum solid
            pat["price_confirmed"] = pat["confirmed"]
            pat["confirmed"] = pat["confirmed"] and vol_ok

    return result

def vol_status_label(pat):
    """
    Label status breakout pola, gabungin konfirmasi price + volume.
    - Price breakout + volume gede (≥ threshold rata2 20 hari) -> CONFIRMED
    - Price udah breakout tapi volume masih kecil -> LOW VOLUME (rawan fakeout)
    - Belum breakout sama sekali -> FORMING
    """
    if pat.get("confirmed"):
        return f"✅ CONFIRMED (Vol {pat.get('vol_ratio', 0):.1f}x)"
    if pat.get("price_confirmed"):
        return f"⚠️ LOW VOL ({pat.get('vol_ratio', 0):.1f}x)"
    return "⏳ FORMING"
...
def fmt_patterns_text(patterns, price_fmt):
    ...
    fw = patterns.get("falling_wedge")
    if fw:
        status = vol_status_label(fw)
        lines.append(f"📉➡️📈 *{fw['type']}* {status} | BO:`{price_fmt(fw['breakout_level'])}`")

    bf = patterns.get("bullish_flag")
    if bf:
        status = vol_status_label(bf)
        lines.append(f"🚩 *{bf['type']}* {status} | BO:`{price_fmt(bf['breakout_level'])}` Target:`{price_fmt(bf['target'])}` Pole:`{bf['pole_gain_pct']:.1f}%`")

    db = patterns.get("double_bottom")
    if db:
        status = vol_status_label(db)
        lines.append(f"〰️ *Double Bottom* {status} | Neck:`{price_fmt(db['neckline'])}` Depth:`{db['depth_pct']:.1f}%`")

    ch = patterns.get("cup_handle")
    if ch:
        status = vol_status_label(ch)
        lines.append(f"☕ *Cup & Handle* {status} | BO:`{price_fmt(ch['breakout_level'])}` Depth:`{ch['cup_depth_pct']:.1f}%`")

    hs = patterns.get("hs")
    if hs:
        status = vol_status_label(hs)
...
def pattern_scan_one(code, tf="D"):
    ...
    try:
        ...
        if pats.get("falling_wedge"): found.append(("falling_wedge", pats["falling_wedge"]))
        if pats.get("bullish_flag"):  found.append(("bullish_flag", pats["bullish_flag"]))
        ...
        is_idr = (r["ticker"].endswith(".JK") or r["ticker"]=="^JKSE")
        return {
...
def check_pattern_breakout(code, tf="D"):
    ...
    try:
        ...
        fw = pats.get("falling_wedge")
        if fw and fw["confirmed"]:
            key = f"{code}_FW"
        ...
        if bf and bf["confirmed"]:
            ...
            if breakout_state_db.get(key) != "confirmed":
                ...
                alerts.append({
                    "code": code, "price": r["price"], "is_idr": is_idr,
                    "msg": (f"🚨 *BREAKOUT ALERT!*\n"
                            f"🚩 *Bullish Flag BREAKOUT*\n"
                            f"*{r['ticker']}* | `{price_fmt(r['price'])}` `{r['chg']:+.2f}%`\n"
                            f"Breakout Level: `{price_fmt(bf['breakout_level'])}` | Target: `{price_fmt(bf['target'])}`\n"
                            f"Volume: `{bf['vol_ratio']:.1f}x` rata2 20D ✅\n"
                            f"Pole: `{bf['pole_gain_pct']:.1f}%` | Trend: {r['trend']} | Score: `{r['score']}/8`\n"
                            f"⏱ {fmt_now()}")
                })
        elif bf and not bf["confirmed"]:
            key = f"{code}_BF"
            if key in breakout_state_db: del breakout_state_db[key]; save_json(BREAKOUT_FILE, breakout_state_db)
            ...
            if breakout_state_db.get(key) != "confirmed":
                ...
                alerts.append({
                    ...
                    "msg": (f"🚨 *BREAKOUT ALERT!*\n"
                            ...
                            f"Volume: `{db['vol_ratio']:.1f}x` rata2 20D ✅\n"
                            f"Trend: {r['trend']} | Score: `{r['score']}/8`\n"
                            f"⏱ {fmt_now()}")
                })
            ...
            if breakout_state_db.get(key) != "confirmed":
                ...
                alerts.append({
                    ...
                    "msg": (f"🚨 *BREAKOUT ALERT!*\n"
                            ...
                            f"Volume: `{ch['vol_ratio']:.1f}x` rata2 20D ✅\n"
                            f"Trend: {r['trend']} | Score: `{r['score']}/8`\n"
                            f"⏱ {fmt_now()}")
                })
            ...
            if breakout_state_db.get(key) != "confirmed":
                ...
                alerts.append({
                    ...
                    "msg": (f"🚨 *BREAKOUT ALERT!*\n"
                            ...
                            f"Volume: `{hs['vol_ratio']:.1f}x` rata2 20D ✅\n"
...
def generate_chart(code, tf="D", volume_spikes=None):
    ...
    # Row 4: Composite Strength heatmap (ala T1MO)
    vol_ma20=pd.Series(vols).rolling(20,min_periods=1).mean().values
    dc_hi_arr=pd.Series(highs).rolling(20,min_periods=1).max().values
    dc_lo_arr=pd.Series(lows).rolling(20,min_periods=1).min().values
    trend_arr=((closes>e9v).astype(float)+(e9v>e20v).astype(float)+
               (e20v>e50v).astype(float)+(closes>e50v).astype(float))/4*100
    macd_norm=np.tanh((macd_v-macd_sig_v)/np.maximum(np.abs(closes)*0.01,1e-6))*50+50
    momentum_arr=np.clip(rsi_v,0,100)*0.6+macd_norm*0.4
    volr_arr=np.clip(np.where(vol_ma20>0,vols/vol_ma20,1),0,3)
    volume_arr=volr_arr/3*100
    dc_pos_arr=np.where(dc_hi_arr>dc_lo_arr,
                         np.clip((closes-dc_lo_arr)/(dc_hi_arr-dc_lo_arr)*100,0,100),50)
    composite_arr=np.clip(trend_arr*0.40+momentum_arr*0.30+volume_arr*0.15+dc_pos_arr*0.15,0,100)

    # ══ LAYOUT ══
    fig=plt.figure(figsize=(14,11),facecolor=BG)
    gs=GridSpec(9,1,figure=fig,height_ratios=[5,1.0,1.0,1.0,0.6,0.28,0.28,0.28,0.28],hspace=0.04)
    ...
    ax_p4=fig.add_subplot(gs[8])  # Composite Strength heatmap
    ...
    # ══ MAGENTA LINE (Hull MA — ala T1MO) ══
    try:
        hma_v=hull_ma(close,20).iloc[-n:].values
        MAGENTA="#e91e63"
        ax1.plot(idx,hma_v,color=MAGENTA,linewidth=1.9,label=f"Magenta:{hma_v[-1]:,.0f}",zorder=8)
    except Exception as hma_err:
        log.warning(f"Hull MA error: {hma_err}")
    ...
    # ══ is_idr & price_fmt — definisi awal ══
    is_idr=(r["ticker"].endswith(".JK") or r["ticker"]=="^JKSE")
    price_fmt=lambda p: f"Rp {p:,.0f}" if is_idr else f"${p:,.2f}"

    # ══ DONCHIAN CHANNEL (T1MO style box) ══
    price_range = float(np.nanmax(highs) - np.nanmin(lows)) or 1.0  # fallback default
    try:
        dc_period = 20
        dc_high_full = df["High"].squeeze()
        dc_low_full  = df["Low"].squeeze()
        dc_upper_s = dc_high_full.rolling(window=dc_period).max().iloc[-n:]
        dc_lower_s = dc_low_full.rolling(window=dc_period).min().iloc[-n:]
        dc_mid_s   = ((dc_upper_s + dc_lower_s) / 2)
        dc_upper_v = dc_upper_s.values
        dc_lower_v = dc_lower_s.values
        dc_mid_v   = dc_mid_s.values
        DC_UPPER_COLOR = "#ef6c00"   # orange — same as Upper TL / T1MO style
        DC_LOWER_COLOR = "#424242"   # dark gray — same as T1MO lower box
        DC_MID_COLOR   = "#90a4ae"   # slate blue-gray for midline
        # Step plot biar keliatan kotak2 kayak T1MO
        ax1.step(list(idx), dc_upper_v, where='post',
                 color=DC_UPPER_COLOR, linewidth=1.8, alpha=0.85, zorder=4,
                 label=f"DC Upper:{price_fmt(dc_upper_v[-1])}")
        ax1.step(list(idx), dc_lower_v, where='post',
                 color=DC_LOWER_COLOR, linewidth=1.8, alpha=0.80, zorder=4,
                 label=f"DC Lower:{price_fmt(dc_lower_v[-1])}")
        ax1.step(list(idx), dc_mid_v, where='post',
                 color=DC_MID_COLOR, linewidth=0.8, linestyle='--', alpha=0.45, zorder=3)
        # Shading inside channel
        ax1.fill_between(list(idx), dc_upper_v, dc_lower_v,
                         step='post', alpha=0.03, color=DC_UPPER_COLOR)
        # Price labels — posisi di KIRI chart (x=1) biar ga nabrak current price label
        price_range = float(np.nanmax(highs) - np.nanmin(lows))
        v_offset = price_range * 0.012  # offset vertikal kecil biar ga numpuk
        ax1.text(1, dc_upper_v[-1] + v_offset,
                 f"DC Upper:{price_fmt(dc_upper_v[-1])}",
                 color=DC_UPPER_COLOR, fontsize=6, va='bottom', fontweight='bold',
                 bbox=dict(boxstyle='round,pad=0.15', facecolor='white',
                           edgecolor=DC_UPPER_COLOR, alpha=0.80, linewidth=0.6),
                 zorder=11)
        ax1.text(1, dc_lower_v[-1] - v_offset,
                 f"DC Lower:{price_fmt(dc_lower_v[-1])}",
                 color=DC_LOWER_COLOR, fontsize=6, va='top', fontweight='bold',
                 bbox=dict(boxstyle='round,pad=0.15', facecolor='white',
                           edgecolor=DC_LOWER_COLOR, alpha=0.80, linewidth=0.6),
                 zorder=11)
    except Exception as dc_err:
        log.warning(f"Donchian Channel error: {dc_err}")
    ...
    # ── Support / Resistance (pivot-based, ala T1MO) ──
    # Cuma 2 level terdekat tiap sisi, garis tipis tanpa kotak teks di harga —
    # teksnya digabung 1 blok compact di posisi TETAP (axes-fraction, kiri-bawah)
    # jadi gak akan pernah nabrak Fib/DC/price-tag label lain.
    try:
        SR_BLUE="#1565c0"; SR_BROWN="#8d6e63"
        sup_levels,res_levels=find_support_resistance(highs,lows,closes,lookback=3,n_levels=2)
        for rv in res_levels:
            ax1.axhline(rv,color=SR_BLUE,linewidth=0.7,linestyle=(0,(4,2)),alpha=0.45,zorder=2)
        for sv in sup_levels:
            ax1.axhline(sv,color=SR_BROWN,linewidth=0.7,linestyle=(0,(4,2)),alpha=0.45,zorder=2)
        sr_txt="  ".join([f"R{i}:{price_fmt(v)}" for i,v in enumerate(res_levels,1)])
        sr_txt2="  ".join([f"S{i}:{price_fmt(v)}" for i,v in enumerate(sup_levels,1)])
        if sr_txt or sr_txt2:
            ax1.text(0.01,0.02,f"📊 {sr_txt}   {sr_txt2}",transform=ax1.transAxes,
                      fontsize=6.5,color=TEXT2,va='bottom',ha='left',
                      bbox=dict(boxstyle='round,pad=0.25',facecolor=BG,edgecolor=GRID,alpha=0.85,linewidth=0.5),
                      zorder=15)
    except Exception as sr_err:
        log.warning(f"Support/Resistance error: {sr_err}")
    ...
    try:
        ...
        if "upper" in tl:
            ...
            ax1.plot([x0, x1],[u["y0"], u["y1"]],
                     ...
                     label="Upper TL")
            # Label harga di ujung Upper TL — digeser biar gak nabrak price-tag di kanan
            lbl_x=min(x1+0.3, n-4)
            lbl_y=u["y1"]
            if abs(lbl_y-lp) < price_range*0.03:
                lbl_y = lp + price_range*0.045 if lbl_y>=lp else lp - price_range*0.045
            ax1.text(lbl_x, lbl_y, f'Rp {u["y1"]:,.0f}'.replace(',','.'),
        ...
        if "lower" in tl:
            ...
            ax1.plot([x0, x1],[lo2["y0"], lo2["y1"]],
                     ...
                     label="Lower TL")
            # Label harga di ujung Lower TL — digeser biar gak nabrak price-tag di kanan
            lbl_x2=min(x1+0.3, n-4)
            lbl_y2=lo2["y1"]
            if abs(lbl_y2-lp) < price_range*0.03:
                lbl_y2 = lp + price_range*0.045 if lbl_y2>=lp else lp - price_range*0.045
            ax1.text(lbl_x2, lbl_y2, f'Rp {lo2["y1"]:,.0f}'.replace(',','.'),
        ...
        if ch:
            ...
            ch_lbl_x=min(ch_offset + ch["cup_right_x"]+1, n-4)
            ch_lbl_y=ch["breakout_level"]*1.002
            if abs(ch_lbl_y-lp) < price_range*0.03:
                ch_lbl_y = lp + price_range*0.045 if ch_lbl_y>=lp else lp - price_range*0.045
            ax1.text(ch_lbl_x, ch_lbl_y,
    ...
    def comp_color(v):
        if v>=60: return "#43a047"   # hijau
        if v>=40: return "#fbc02d"   # kuning
        return "#e53935"             # merah
    for i in idx:
        ax_p4.bar(i,1.0,bottom=0,color=comp_color(composite_arr[i]),width=0.92,zorder=3)
    ax_p4.set_ylim(0,1.4); ax_p4.set_xlim(-0.5,n-0.5)
    ax_p4.set_yticks([]); ax_p4.set_xticks([])
    ax_p4.text(-0.5,0.5,"STR",color=TEXT2,fontsize=5.5,va='center',ha='right',fontweight='bold')
...
async def chart_cmd(u,c):
    ...
    is_idr=(r.get("ticker","").endswith(".JK") or r.get("ticker","")=="^JKSE")
    price_str=f"Rp {r['price']:,.0f}" if is_idr else f"${r['price']:,.2f}"
    price_fmt=lambda p: f"Rp {p:,.0f}" if is_idr else f"${p:,.2f}"
    ts = calculate_tp_sl(r)
    pat_text = ""
    ...
    caption=(f"*{r['ticker']}* | TF:`{tf}` | `{price_str}` `{r['chg']:+.2f}%`\n"
             ...
             f"💠 Strength:`{r.get('composite','-')}` {r.get('composite_label','')}\n"
...
async def screener_cmd(u,c):
    ...
    for r in res[:15]:
        ...
        lines.append(f"{em} *{r['code']}* `{r['price']:,.0f}` {r['chg']:+.2f}% Score:`{r['score']}/8` {top}{vs}{liq} | 💠`{r.get('composite','-')}`{r.get('composite_label','')[:2]}")
...
async def screener_us_exec(u,c,ms=2):
    ...
    for r in res[:12]:
        ...
        top=r["sigs"][0].split("-")[0].strip() if r["sigs"] else "—"
        vs="🌊" if r["vr"]>=2 else ""
        lines.append(f"{em} *{r['code']}* `${r['price']:,.2f}` {r['chg']:+.2f}% Score:`{r['score']}/8` {top}{vs} | 💠`{r.get('composite','-')}`{r.get('composite_label','')[:2]}")
    lines+=["━━━━━━━━━━━━━━━━━━━━",f"⏱ {fmt_now()}"]
    await m.edit_text("\n".join(lines),parse_mode="Markdown")
...
async def doji_cmd(u,c):
    ...
    try:
        ...
        if best:
            ...
            if buf:
                is_idr = (best["ticker"].endswith(".JK") or best["ticker"]=="^JKSE")
...
async def volmom_auto_scan(context):
    ...
    for uid in auto_users:
        try:
            ...
            if buf:
                is_idr = (hot[0]["ticker"].endswith(".JK") or hot[0]["ticker"]=="^JKSE")
...
async def volume_cmd(u,c):
    ...
    def fetch_vol(code):
        try:
            df=get_cached_data(f"{code}.JK","1d","5d")  # ✅ FIX: pakai cache, bukan yf.download mentah
            if len(df)>=2:
        ...
        return None
    with ThreadPoolExecutor(max_workers=6) as ex:  # ✅ FIX: 10→6, spike CPU lebih landai
...
async def trend_cmd(u,c):
    ...
    try:
        ih=get_cached_data("^JKSE","1d","5d")  # ✅ FIX: pakai cache, bukan yf.download mentah
...
async def pattern_cmd(u, c):
    ...
    label = "US" if market == "us" else "IDX"
    m = await u.message.reply_text(
        "Scanning Pattern " + label + "... Triangle | Bullish Flag | Double Bottom | Cup&Handle | H&S\nHarap tunggu...",
        parse_mode="Markdown")
    try:
        results = await asyncio.get_event_loop().run_in_executor(
            None, parallel_pattern_scan, stocks, "D")
        ...
        ch_list  = [r for r in results if any(p[0]=="cup_handle"    for p in r["patterns"])]
        tri_list = [r for r in results if any(p[0]=="triangle"      for p in r["patterns"])]
        fw_list  = [r for r in results if any(p[0]=="falling_wedge" for p in r["patterns"])]
        bf_list  = [r for r in results if any(p[0]=="bullish_flag"  for p in r["patterns"])]
        hs_list  = [r for r in results if any(p[0]=="hs"            for p in r["patterns"])]
        if db_list:
            ...
            for r in db_list[:5]:
                ...
                db=next(p[1] for p in r["patterns"] if p[0]=="double_bottom")
                st="\u2705" if db["confirmed"] else ("\u26a0\ufe0f" if db.get("price_confirmed") else "\u23f3")
                liq="\u26a0\ufe0f" if not r.get("liquid",True) else ""
        ...
        if ch_list:
            ...
            for r in ch_list[:5]:
                ...
                px="Rp {:,.0f}".format(r["price"]) if is_idr else "${:,.2f}".format(r["price"])
                ch2=next(p[1] for p in r["patterns"] if p[0]=="cup_handle")
                st="\u2705" if ch2["confirmed"] else ("\u26a0\ufe0f" if ch2.get("price_confirmed") else "\u23f3")
        ...
        if fw_list:
            lines_msg.append("\n\U0001f4c9\u27a1\ufe0f\U0001f4c8 *FALLING WEDGE (" + str(len(fw_list)) + "):*")
            for r in fw_list[:5]:
                is_idr=r["is_idr"]
                px="Rp {:,.0f}".format(r["price"]) if is_idr else "${:,.2f}".format(r["price"])
                fw2=next(p[1] for p in r["patterns"] if p[0]=="falling_wedge")
                st="\u2705" if fw2["confirmed"] else ("\u26a0\ufe0f" if fw2.get("price_confirmed") else "\u23f3")
                liq="\u26a0\ufe0f" if not r.get("liquid",True) else ""
                lines_msg.append("  " + st + " *" + r["code"] + "* `" + px + "` {:+.2f}% {} ".format(r["chg"],fw2["quality"]) + liq)
        if bf_list:
            lines_msg.append("\n\U0001f6a9 *BULLISH FLAG (" + str(len(bf_list)) + "):*")
            for r in bf_list[:5]:
                is_idr=r["is_idr"]
                px="Rp {:,.0f}".format(r["price"]) if is_idr else "${:,.2f}".format(r["price"])
                bf2=next(p[1] for p in r["patterns"] if p[0]=="bullish_flag")
                st="\u2705" if bf2["confirmed"] else ("\u26a0\ufe0f" if bf2.get("price_confirmed") else "\u23f3")
                lines_msg.append("  " + st + " *" + r["code"] + "* `" + px + "` {:+.2f}% Pole:{:.1f}% Target:".format(r["chg"],bf2["pole_gain_pct"]) + ("Rp {:,.0f}".format(bf2["target"]) if is_idr else "${:,.2f}".format(bf2["target"])))
        if hs_list:
        ...
        try:
            ...
            for uid in list(auto_users.keys()):
                try:
                    ...
                    if buf:
                        is_idr = best_(r["ticker"].endswith(".JK") or r["ticker"]=="^JKSE")
...
def detect_first_green(code, tf="D"):
    ...
    try:
        ...
        e50     = r["e50"]
        ticker  = r["ticker"]
        is_idr  = (ticker.endswith(".JK") or ticker=="^JKSE")
        liquid  = r.get("liquid", True)
...
def generate_generic_screener_image(rows, cols, title="SCREENER", market="IDX", footer_note=""):
    ...
    try:
        ...
        row_h  = 0.55   # ✅ naik dari 0.48
        hdr_h  = 0.88   # ✅ naik dari 0.80
        foot_h = 0.55
        total_h = hdr_h + 0.40 + n * row_h + foot_h + 0.3
        fig_w   = 11.0  # ✅ naik dari 10.0
        ...
        # ── Header ──
        y = total_h - 0.08
        ax.add_patch(plt.Circle((0.22, y - 0.16), 0.09, color=GREEN, zorder=3))
        ax.text(0.40, y - 0.06, "IDX QUANT  DANTEBADAI BOT",
                color=GREEN, fontsize=11.5, fontweight="bold", va="top", ha="left", fontfamily="monospace")
        ax.text(0.40, y - 0.34, f"{flag}  {title}",
                color=SUBTEXT, fontsize=9.5, va="top", ha="left")
        ax.text(fig_w - 0.20, y - 0.06, now_str,
                color=CYAN, fontsize=9.5, va="top", ha="right", fontfamily="monospace")
        ax.plot([0.1, fig_w - 0.1], [y - 0.52, y - 0.52], color=BORDER, linewidth=0.8)
        ...
        for i, (lbl, key, wr, align, cfn) in enumerate(cols):
            ax.text(x_pos[i], y_hdr, lbl,
                    color=SUBTEXT, fontsize=8.5, fontweight="bold",
                    va="top", ha=align, fontfamily="monospace")
        ax.plot([0.1, fig_w - 0.1], [y_hdr - 0.22, y_hdr - 0.22], color=BORDER, linewidth=0.6)
        ...
        for idx, row in enumerate(rows):
            ...
            for i, (lbl, key, wr, align, cfn) in enumerate(cols):
                ...
                if key == "score_bar":
                    ...
                    score_mode = row.get("score_mode", "bullish")  # "bullish" or "overbought"
                    dot_w = 0.10; gap = 0.026
                    total_dw = max_s * dot_w + (max_s - 1) * gap
                    x0 = x_pos[i]
                    for si in range(max_s):
                        xi = x0 + si * (dot_w + gap)
                        if score_mode == "overbought":
                            # Pivot style: tinggi = merah (overbought/kena TL)
                            if score >= 8:     sc_col = RED
                            elif score >= 6:   sc_col = ORANGE
                            elif score >= 4:   sc_col = YELLOW
                            else:              sc_col = GREEN
                        else:
                            # Bullish style: tinggi = ijo (makin bagus makin ijo)
                            if score >= 8:     sc_col = GREEN
                            elif score >= 6:   sc_col = "#68d391"   # hijau muda
                            elif score >= 4:   sc_col = YELLOW
                            else:              sc_col = ORANGE
                        fc = sc_col if si < score else BORDER
                        ax.add_patch(mpatches.FancyBboxPatch(
                            (xi, y_mid - 0.07), dot_w, 0.14,
                            boxstyle="round,pad=0.01", facecolor=fc, edgecolor="none"))
                    ax.text(x0 + total_dw + 0.08, y_mid, f"{score}/{max_s}",
                            color=sc_col if score else DIM, fontsize=8,
        ...
        # ── Footer ──
        ax.plot([0.1, fig_w - 0.1], [0.40, 0.40], color=BORDER, linewidth=0.6)
        ax.text(0.20, 0.30, footer_note or "Dantebadai Bot v2",
                color=DIM, fontsize=8, va="top", ha="left")
        ax.text(fig_w - 0.20, 0.30, "Dantebadai Bot v2",
                color=DIM, fontsize=8, va="top", ha="right")
...
async def pivot_auto_scan(context):
    ...
    for r in lower_res[:8]:
        ...
        is_idr_s = (ticker.endswith(".JK") or ticker=="^JKSE")
    ...
    for uid in auto_users:
        try:
            ...
            if img_buf:
                ...
                if is_idx_market_open():
                    idx_hits = [r for r in lower_res if (r["ticker"].endswith(".JK") or r["ticker"]=="^JKSE")]
                    best = idx_hits[0] if idx_hits else lower_res[0]
                else:
                    us_hits = [r for r in lower_res if not (r["ticker"].endswith(".JK") or r["ticker"]=="^JKSE")]
                ...
                if not same or stale:
                    ...
                    if buf:
                        ...
                        is_idr_b = (ticker_b.endswith(".JK") or ticker_b=="^JKSE")
